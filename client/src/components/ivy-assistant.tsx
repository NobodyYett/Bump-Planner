...
export function IvyAssistant() {
  ...
  async function handleAsk(e: React.FormEvent) {
    ...
    if (error) {
      console.error(error);
      setErrorMsg(
        "Ivy couldn't answer right now. Please try again in a moment."
      );
      return;
    }
  ...
  return (
    <div className="mt-10 bg-card rounded-3xl border border-border shadow-sm p-6 space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div>
          <h2 className="font-serif text-xl font-semibold">Ask Ivy</h2>
      ...
      <form onSubmit={handleAsk} className="space-y-3">
        <div className="flex flex-col gap-2">
          <label className="text-xs font-medium text-muted-foreground">
            What would you like to ask Ivy?
          </label>
      ...
      {answer && (
        <div className="mt-3 rounded-2xl bg-muted/70 border border-border px-4 py-3">
          ...
          <p className="mt-2 text-[10px] text-muted-foreground">
            Ivy shares general education & emotional support only. For anything
